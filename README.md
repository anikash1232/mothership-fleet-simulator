...
The interface defines the functionality necessary for all of our modules.  We can infer that there will be a name instance variable.  Your job is to create the infrastructure necessary to create a category of modules that are interchangeable via dependency injection. To implement the IModule interface, we will create an abstract parent class to hold all of our information.  
...
1. Constructor: SolarGenerator extends AModule, and you will have to make a constructor to initialize the object.
...
Thought experiment: Why is ArrayList bound to `IModule`?  In this case the java generics allow us to hold all the objects that have an **IS-A** relationship with IModule which is what we want.  We could also choose AModule, but that would tightly couple our implementation to existing classes.  Interfaces are more flexible.  
...
## Merlin: Extra Credit
[Note: You are only eligible for extra credit if you have 100% on the rest]
